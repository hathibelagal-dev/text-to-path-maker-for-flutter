...
class PMFont {
  ...
  /*
  List<Segment> generateSegments(cIndex) {
    var segments = [];
    var glyphId = -1;
    for (var i = 0; i < tables['glyf'].data['glyphs'].length; i++) {
      if (tables['cmap'].data['characterMap'][i] == cIndex) {
    ...
    var path = "";
    List<SubCharacter> subCharacters = [];
    for (var k = 0; k < contours.length; k++) {
      var contour = contours[k];
      ...
      for (var i = 0; i < interpolated.length - 1; i++) {
        if (i == 0) {
          prev = Offset(interpolated[i].x, interpolated[i].y);
        } else {
          if (!interpolated[i].isOnCurve) {
            var start = prev;
            var controlPoint = Offset(interpolated[i].x, interpolated[i].y);
            var end = Offset(interpolated[i + 1].x, interpolated[i + 1].y);

            subCharacters.add(SubCharacter.createSubCharacterQuadBezier(
                start, controlPoint, end));

            prev = Offset(interpolated[i + 1].x, interpolated[i + 1].y);
            i++;
          } else {
            var start = prev;
            var end = Offset(interpolated[i].x, interpolated[i].y);
            subCharacters.add(SubCharacter.createSubCharacterLine(start, end));
            prev = Offset(interpolated[i].x, interpolated[i].y);
          }
        }
        pos = i;
      ...
      if ((pos + 1) < interpolated.length) {
        var start = prev;
        var end = Offset(interpolated[pos + 1].x, interpolated[pos + 1].y);
        subCharacters.add(SubCharacter.createSubCharacterLine(start, end));
      }
    }
    var description = CharacterDescription(subCharacters, Offset(0, 0), 5);
    var character = Character(description: description);

    return character.getCharSegments();
  }
  */
}
